...
SeqTrees is based on sequential synthesis of tabular data. Instead of learning
...
In SeqTrees, the first variable is sampled from an empirical marginal
...
observed integer codes. For continuous variables, SeqTrees can either sample
...
[@khaled_el_mosquera_zheng_2020]. SeqTrees supports three options.

### Use Input Column Order

If `variable_order` is not supplied and `optimize_order=False`, SeqTrees uses the
...
Use `optimize_order=True` when you want SeqTrees to choose a greedy order from
the data.
...
SeqTrees first chooses the variable with the smallest empirical entropy:
...
After the first variable, SeqTrees uses a greedy forward selection rule. For
